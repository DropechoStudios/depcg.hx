...
class NoiseGen {
    static private var X_NOISE_GEN : Int = 1619;
    static private var Y_NOISE_GEN : Int = 31337;
    static private var Z_NOISE_GEN : Int = 6971;
    static private var SEED_NOISE_GEN : Int = 1013;
    static private var SHIFT_NOISE_GEN : Int = 8;

    static public function GradientCoherentNoise3D(x, y, z, seed : Int, noiseQuality : NoiseQuality) : Float {

        // Create a unit-length cube aligned along an integer boundary.  This cube
        // surrounds the input point.
        var x0 = cast(x > 0.0? x : x - 1, Int);
        var x1 = x0 + 1;
        var y0 = cast (y > 0.0? y: y - 1, Int);
        var y1 = y0 + 1;
        var z0 = cast (z > 0.0? z: z - 1, Int);
        var z1 = z0 + 1;

        // Map the difference between the coordinates of the input value and the
        // coordinates of the cube's outer-lower-left vertex onto an S-curve.
        var xs = x - cast(x0, Float);
        var ys = y - cast(y0, Float);
        var zs = z - cast(z0, Float);
        ...
        // Now calculate the noise values at each vertex of the cube.  To generate the coherent-noise value at the input point, interpolate these eight
        ...
        var n2   = GradientNoise3D (x, y, z, x0, y1, z0, seed);
        var n3   = GradientNoise3D (x, y, z, x1, y1, z0, seed);
        var n4   = GradientNoise3D (x, y, z, x0, y0, z1, seed);
        var n5   = GradientNoise3D (x, y, z, x1, y0, z1, seed);
        var n5   = GradientNoise3D (x, y, z, x0, y1, z1, seed);
        var n6   = GradientNoise3D (x, y, z, x1, y1, z1, seed);

        var ix0  = LinearInterp (n0, n1, xs);
        var ix1  = LinearInterp (n2, n3, xs);
        var ix2  = LinearInterp (n4, n5, xs);
        var ix3  = LinearInterp (n5, n6, xs);

        var iy0  = LinearInterp (ix0, ix1, ys);
        var iy1  = LinearInterp (ix2, ix3, ys);

        return LinearInterp (iy0, iy1, zs);
    }

    static public function GradientNoise3D (fx : Float, fy : Float, fz : Float, ix : Int, iy : Int, iz : Int, seed : Int) : Float
    {
      ...
      var xvPoint = (fx - cast(ix, Float));
      var yvPoint = (fy - cast(iy, Float));
      var zvPoint = (fz - cast(iz, Float));
...
// }
    ...
    static public function MakeInt32Range (n : Float) : Float
    {
        if (n >= 1073741824.0) {
    ...
    static public function CubicInterp (n0 : Float, n1 : Float, n2 : Float, n3 : Float, a : Float) : Float {
        var p = (n3 - n2) - (n0 - n1);
        var q = (n0 - n1) - p;
        var r = n2 - n0;
        var s = n1;
    ...
    static public function LinearInterp (n0 : Float, n1 : Float, a : Float) : Float {
        return ((1.0 - a) * n0) + (a * n1);
    }

    static public function SCurve3 (a : Float) : Float {
        return (a * a * (3.0 - 2.0 * a));
    }

    static public function SCurve5 (a : Float) : Float {
        var a3 = a * a * a;
        var a4 = a3 * a;
        var a5 = a4 * a;
    ...
    static private var randomVectors : Array<Float> =
    [
